    ...
    -- Library element functions
        function library:window(properties)
            local cfg = { 
                ...
                size = properties.size or properties.Size or dim2(0, 600, 0, 500);
            ...
            local items = cfg.items; do
                ...
                items[ "tab_listlayout" ] = library:create( "UIListLayout" , {
                ...
                -- items[ "tab_listlayout" ]:GetPropertyChangedSignal("AbsoluteContentSize"), function()
                --     if items[ "button_holder" ].CanvasSize ~= UDim2.fromOffset(0, items[ "tab_listlayout" ].AbsoluteContentSize.Y + 2) then
                --         items[ "button_holder" ].CanvasSize = UDim2.fromOffset(0, items[ "tab_listlayout" ].AbsoluteContentSize.Y + 2)
                --     end
                -- end)

                items[ "tab_listlayout" ]:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                    if items[ "button_holder" ].CanvasSize ~= UDim2.fromOffset(0, items[ "tab_listlayout" ].AbsoluteContentSize.Y + 20) then
                        items[ "button_holder" ].CanvasSize = UDim2.fromOffset(0, items[ "tab_listlayout" ].AbsoluteContentSize.Y + 20)
                    end
                end)
            end 
        ...
        function library:tab(properties)
            ...
            return #cfg.pages > 1 and cfg.pages or unpack(cfg.pages)
        end
        ...
        function library:dropdown(options) 
            ...
            function cfg.set_visible(bool)
                ...
                items[ "dropdown_holder" ].Position = dim2(0, cfg.multi and items[ "dropdown" ].AbsolutePosition.X or items[ "dropdown" ].AbsolutePosition.X + 1, 0, items[ "dropdown" ].AbsolutePosition.Y + 84)
        ...
        function save_config()
            if not library.vip then return end
            writefile(library.directory .. "/configs/" .. game.Players.LocalPlayer.Name .. ".cfg", library:get_config())
        end

        function library:auto_load_config()
            if not library.vip then return end
            if isfile(`{library.directory}/configs/{game.Players.LocalPlayer.Name}.cfg`) then
                library:load_config(readfile(`{library.directory}/configs/{game.Players.LocalPlayer.Name}.cfg`))
            end
        ...
        function library:init_config(window) 
            ...
            local section = column:section({name = "Servers", size = 1, default = true, icon = "rbxassetid://139628202576511"})
            section:textbox({name = "Join by Job ID", flag = "jobId"})
            section:button({ name = "Join Server", callback = function()
                local jobId = flags.jobId
                ...
                if jobId and jobId ~= "" then
                    ...
                    if not success then
                        notifications:create_notification({
                            name = "Error Joining Server",
                            info = "Could not join the server: " .. tostring(result)
                        })
                    else
                        notifications:create_notification({
                            name = "Joining Server",
                            info = "Attempting to join server " .. tostring(jobId) .. "."
                        })
                    end
                else
                    notifications:create_notification({
                        name = "Invalid Job ID",
                        info = "Please enter a valid Job ID."
                    })
                end
            end})
            section:button({ name = "Copy Current Job ID", callback = function() setclipboard(game.JobId) notifications:create_notification({name = "Copied Job ID", info = "The current server's Job ID has been copied to your clipboard." }) end })
            ...
            gui_service.NativeClose:Connect(save_config)
            gui_service.MenuOpened:Connect(save_config)
